...
with tf.name_scope("channel_sensing"):
    ...
    for t in range(tau): 
        ...
        RSS_val = tf.math.square(tf.abs(y_complex))/tf.sqrt(lay['P'])
        if t == 0:
            input_ = RSS_val
        else:
            input_ = tf.concat([input_, RSS_val],axis = 1)
...
with tf.Session() as sess:
    if initial_run == 1:
        ...
        saver.restore(sess, './loc/DNN_fixSNR_learn/params_DNN_loc_RIS_learn_closeBS_fullRician_RSS_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0])+'_rician_' + str(Rician_factor))
    ...
    for epoch in range(n_epochs):
        ...
        if epoch%5==1: #Every 5 iterations it checks if the validation performace is improved, then saves parameters
            if loss_val < best_loss:
                save_path = saver.save(sess, './loc/DNN_fixSNR_learn/params_DNN_loc_RIS_learn_closeBS_fullRician_RSS_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0])+'_rician_' + str(Rician_factor))
                best_loss = loss_val
...
plt.show()
sio.savemat('./loc/DNN_fixSNR_learn/data_DNN_loc_RIS_learn_closeBS_fullRician_RSS_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0]) +'_rician_' + str(Rician_factor) +'.mat',dict(performance= performance,\
