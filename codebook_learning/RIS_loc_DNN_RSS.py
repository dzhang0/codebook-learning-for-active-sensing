...
with tf.Session() as sess:
    if initial_run == 1:
        ...
        saver.restore(sess, './loc/DNN_fixSNR/params_DNN_loc_RIS_closeBS_fullRician_RSS_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0])+'_rician_' + str(Rician_factor))
    ...
    for epoch in range(n_epochs):
        ...
        if epoch%5==1: #Every 5 iterations it checks if the validation performace is improved, then saves parameters
            if loss_val < best_loss:
                save_path = saver.save(sess, './loc/DNN_fixSNR/params_DNN_loc_RIS_closeBS_fullRician_RSS_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0])+'_rician_' + str(Rician_factor))
                best_loss = loss_val
...
plt.show()
sio.savemat('./loc/DNN_fixSNR/data_DNN_loc_RIS_new_closeBS_fullRician_RSS_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0]) +'_rician_' + str(Rician_factor) +'.mat',dict(performance= performance,\
