...
def generate_RSS_adaptive(A_T_real,the_theta, P_temp):
    ...
    for tau_i in range(tau):
        ...
        noise =  np.random.normal(size = np.shape(theta_A_k_T_re), loc = 0.0, scale = noiseSTD_per_dim) + \
                    1j*np.random.normal(size = np.shape(theta_A_k_T_re), loc = 0.0, scale = noiseSTD_per_dim)
        RSS_i =  abs( (np.sqrt(P_temp)+ 1j*0.0) *(theta_A_k_T_re + 1j*theta_A_k_T_im)+ noise) ** 2 
        print(RSS_i.shape)
...
with tf.name_scope("channel_sensing"):
    ...
    theta_list = []
    for t in range(tau): 
        ris_t_re = tf.get_variable("wa_uplink_real2" + str(t), shape=(1, N_ris), trainable=True)
        ris_t_im = tf.get_variable("wa_uplink_imag2" + str(t), shape=(1, N_ris), trainable=True)
        ris_t_complex = tf.complex(ris_t_re, ris_t_im)
        v = ris_t_complex / tf.cast(tf.abs(ris_t_complex),tf.complex64)
        #v = ris_t_complex / tf.cast(ris_t_complex, axis=1, keepdims=True)

        theta_T_tau = tf.concat([tf.real(v),tf.imag(v)],axis=1)

        theta_list.append(theta_T_tau)
...
with tf.Session() as sess:
    if initial_run == 1:
        ...
        saver.restore(sess, './loc/DNN_fixSNR_learn/params_DNN_loc_RIS_learn_closeBS_fullRician_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0])+'_rician_' + str(Rician_factor))
    ...
    for epoch in range(n_epochs):
        ...
        if epoch%5==1: #Every 5 iterations it checks if the validation performace is improved, then saves parameters
            if loss_val < best_loss:
                save_path = saver.save(sess, './loc/DNN_fixSNR_learn/params_DNN_loc_RIS_learn_closeBS_fullRician_3D_tau_'+ str(tau) +'_snr'+str(snr_const[0])+'_rician_' + str(Rician_factor))
                best_loss = loss_val
    ...
    for j in range(scale_factor):
        ...
        coordinate_k = np.array([25, -15, -20])
        ...
        mse_loss,loc_hat_test, theta_test = sess.run([loss,loc_hat, theta_list],feed_dict=feed_dict_test)
...
plt.show()
sio.savemat('./plot/interpret/interpret_RNN_learn_RIS_tau_'+ str(tau) +'_snr'+str(snr_const[0]) +'_rician_' + str(Rician_factor) +'.mat',dict(performance= performance,\
